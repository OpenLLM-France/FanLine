...
| `/queue/status/` payload = id | GET | Obtenir le statut | `{"status": "waiting", "position": 5, "timestamp": "2024-01-20 15:30:00", "remaining_time": 60}` ou si erreur `{"status": "error", "message": "message d'erreur"}` |
...
| `/queue/get_users` | GET | Obtenir les utilisateurs actifs et en draft | `{"active_users": 50, "waiting_users": 5, "draft_users": ["user1", "user2"], "total_slots": 50, "total_accounts": 100}` |
...
| `/queue/update_draft_duration` payload = duration | POST | Mettre à jour la durée du draft | `{"status": "success"}` |
| `/health` | GET | Vérifier la santé du service | `{"status": "success"}` |
| `/queue/leave` | POST | Quitter la file | `{"status": "success"}` |
| `/queue/update_draft_duration` payload = duration | POST | Mettre à jour la durée du draft | `{"status": "success"}` |
| `/queue/update_session_duration` payload = duration | POST | Mettre à jour la durée de la session | `{"status": "success"}` |
| `/queue/update_max_users` payload = max_users | POST | Mettre à jour le nombre maximum d'utilisateurs | `{"status": "success"}` |
